      ...
      fields.forEach(function(field) {
        ...
        if (isRequired && !value) {
          ...
          if (!emailRegex.test(value)) {
            errorMsg = field.getAttribute('data-error-email') || 'Please enter a valid email';
          }
          ...
          if (value.length < min) {
            errorMsg = field.getAttribute('data-error-minlength') || 'Minimum ' + min + ' characters required';
          }
        }
      ...
      if (type === 'email' && value) {
        ...
        if (!emailRegex.test(value)) {
          ...
          if (!existingError) {
            var errorEl = document.createElement('span');
            errorEl.className = 'form-error';
            errorEl.textContent = field.getAttribute('data-error-email') || 'Please enter a valid email';
            field.parentNode.appendChild(errorEl);
          }
        } else {
          field.classList.remove('error');
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '7f774bbf-ac9f-453c-9a6a-e9d1f0ef8440',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '7f774bbf-ac9f-453c-9a6a-e9d1f0ef8440';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '7f774bbf-ac9f-453c-9a6a-e9d1f0ef8440';
